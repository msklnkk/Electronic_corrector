# # backend/src/project/api/report_routes.py
...
# )
# async def get_report_by_id(id: int) -> ReportSchema:
#     try:
#         async with database.session() as session:
#             report = await report_repo.get_report_by_id(session=session, report_id=id)
...
#     dependencies=[Depends(check_for_admin_access)],
# )
# async def get_reports_by_check_id(id: int) -> list[ReportSchema]:
#     async with database.session() as session:
#         reports = await report_repo.get_reports_by_check_id(session=session, check_id=id)
#         if not reports:
#             raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Отчёты для check_id={id} не найдены")
#         return reports
#
# @report_routes.post(
#     "/reports",
...
#     try:
#         async with database.session() as session:
#             await report_repo.delete_report(session=session, report_id=id)
#     except ReportNotFound as error:
#         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=error.message)